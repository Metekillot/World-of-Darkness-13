...
/datum/species/ghoul/spec_life(mob/living/carbon/human/ghoul)
	. = ..()
	if(HAS_TRAIT(ghoul, TRAIT_UNMASQUERADE))
		if(ghoul.CheckEyewitness(ghoul, ghoul, 7, FALSE))
			ghoul.AdjustMasquerade(-1)
	if(istype(get_area(ghoul), /area/vtm))
		var/area/vtm/masqueraded_area = get_area(ghoul)
		if(masqueraded_area.zone_type == "masquerade" && masqueraded_area.upper)
			if(ghoul.pulling)
				if(ishuman(ghoul.pulling))
					var/mob/living/carbon/human/pull = ghoul.pulling
					if(pull.stat == DEAD)
						var/obj/item/card/id/id_card = ghoul.get_idcard(FALSE)
						if(!istype(id_card, /obj/item/card/id/clinic) && !istype(id_card, /obj/item/card/id/police) && !istype(id_card, /obj/item/card/id/sheriff) && !istype(id_card, /obj/item/card/id/prince) && !istype(id_card, /obj/item/card/id/camarilla))
							if(ghoul.CheckEyewitness(ghoul, ghoul, 7, FALSE))
								if(ghoul.last_loot_check+50 <= world.time)
									ghoul.last_loot_check = world.time
									ghoul.last_nonraid = world.time
									ghoul.killed_count = ghoul.killed_count+1
									if(!ghoul.warrant && !ghoul.ignores_warrant)
										if(ghoul.killed_count >= 5)
											ghoul.warrant = TRUE
											SEND_SOUND(ghoul, sound('code/modules/wod13/sounds/suspect.ogg', 0, 0, 75))
											to_chat(ghoul, "<span class='userdanger'><b>POLICE ASSAULT IN PROGRESS</b></span>")
										else
											SEND_SOUND(ghoul, sound('code/modules/wod13/sounds/sus.ogg', 0, 0, 75))
											to_chat(ghoul, "<span class='userdanger'><b>SUSPICIOUS ACTION (corpse)</b></span>")
			for(var/obj/item/I in ghoul.contents)
				if(I)
									...
									if(ghoul.last_loot_check+50 <= world.time)
										...
										if(!ghoul.warrant && !ghoul.ignores_warrant)
											if(ghoul.killed_count >= 5)
												...
												SEND_SOUND(ghoul, sound('code/modules/wod13/sounds/sus.ogg', 0, 0, 75))
												to_chat(ghoul, "<span class='userdanger'><b>SUSPICIOUS ACTION (equipment)</b></span>")
	if(ghoul.key && ghoul.stat != DEAD)
...
/datum/species/human/spec_life(mob/living/carbon/human/H)
	...
	if(istype(get_area(H), /area/vtm))
		var/area/vtm/masqueraded_area = get_area(H)
		if(masqueraded_area.zone_type == "masquerade" && masqueraded_area.upper)
			if(H.pulling)
